    ...
    try {
        ...
        if (user && user.credits < 5) {
            return res.status(403).json({
                message: "Add credits to keep the creativity flowing on your next project.",
            });
        }
        ...
        await prisma_1.default.conversation.create({
            data: {
                role: "assistant",
                content: "We're crafting your website as we speak...",
                projectId: project.id,
            },
        });
        ...
        await prisma_1.default.conversation.create({
            data: {
                role: "assistant",
                content: "Here is your website! You can now review it and request any changes",
                projectId: project.id,
            },
        });
...
const purchaseCredits = async (req, res) => {
    try {
        ...
        if (!plan) {
            return res.status(404).json({ message: "Plan is required" });
        }
            ...
            line_items: [
                {
                    price_data: {
                        currency: "usd",
                        product_data: {
                            name: `EasyAI - ${plan.credits} credits`,
                        },
